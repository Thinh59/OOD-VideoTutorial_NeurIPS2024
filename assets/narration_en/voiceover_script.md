## Scene 0.1 - Opening: Clinical Notes
...
## Scene 1.1 - ERM and the Accuracy Illusion
...
## Scene 1.3 - What Is a Spurious Feature?
...
## Scene 1.31 - Formalizing Variables
...
## Scene 1.32 - ID and OOD Distributions
...
## Scene 1.33 - Environment Formalization
...
## Scene 1.4 - Geometry and Inductive Bias
...
## Scene 2.0 - Risk Aggregation Families
...
## Scene 2.2 - Worst-Group Accuracy
...
## Scene 2.3 - Distribution Shift Types
...
## Scene 3.2 - Shift Breaks Shortcut
...
## Scene 4.0 - Importance Weighting and Interpolation
...
## Scene 6.0 - Semantic Corruptions
...
## Scene 7.1 - Scale Does Not Solve It
...
## Scene 7.2 - CLIP and Web Correlations
...
## Scene 7.5 - Prompting for Robustness
...
## Scene 8.2 - Model Selection Paradox
...
## Scene 1.3A - Variables: X, Y, E

In machine learning, we have X as input and Y as label. But we often forget E, the environment. Environment dictates how X and Y relate.

## Scene 1.3B - OOD Shift Breaks Assumptions

Standard ML assumes P-train equals P-test. But OOD shift breaks this. The training and test distributions are totally different.

## Scene 1.3C - Distribution Set

Instead of one distribution, we train on a set of environments, hoping for stability in a new one.

## Scene 1.4B - Geometric Skew

SGD is lured by the majority group, creating a max-margin boundary that skewers the minority.

## Scene R4 - Risk Aggregation

Different methods like ERM, DRO, and CVaR aggregate risk differently, but they all share the goal of robustness.

## Scene 3.3 - The Ultimate Test

When the environment shifts, only the causal model survives. The spurious model completely fails.

## Scene 4.1A - The Invariance Principle

To be robust, the optimal classifier must be the same across all environments.

## Scene 4.1B - Finding the Invariant Space

We project distorted environments into a shared invariant space where one rule works for all.

## Scene N1.5 - NuRD Divergence

NuRD uses a divergence penalty to force distributions to match perfectly, stripping away nuisance features.

## Scene RW4 - Avoiding Interpolation

To avoid memorization, we can undersample the majority class, or use a two-stage approach by freezing the feature extractor.

## Scene 9.1 - The Journey Remastered

Shortcuts still exist in modern AI. Our goal is to redefine what's easy, achieving ultimate stability.

## Scene 9.1B - The Robustness Triangle

ERM, DRO, and Invariance form the robustness triangle, all pointing towards STABILITY.

## Scene 9.2 - Open Problems and Credits
